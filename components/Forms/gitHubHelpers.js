...
export const publish = (values) => {
    getToken(values)
        .then(response => {
            ...
            const { author, title, sample, overwrite } = values;
...
`;
            ...
            ghRequest.repos(`${username}`, `${repo}`)
                .contents(`pages/articles/${path}/index.md`)
                .add(config)
                .then((res) => {
...
export const saveToGitHub = (values) => {
    getToken(values)
        .then(response => {
            const { token } = response;
            const { username, repo, path, file, overwrite } = values;
            const isMarkdown = file.slice(-3) === '.md'
            const rawEditorContent = localStorage.getItem('rawEditorContent')
            const contentToSend = isMarkdown ?
                stateToMarkdown(convertFromRaw(JSON.parse(rawEditorContent))) :
                rawEditorContent
...
export const loadFromGitHub = (values, dispatch, props) => {
  ...
  // console.log('props-load:', props)
    getToken(values)
        .then(response => {
            const { token } = response;
            const { username, repo, path, file } = values;
            const { passContent } = props;

            const ghRequest = new Octokat({ token: `${token}` });
            ghRequest.repos(`${username}`, `${repo}`)
                .contents(`${path}/${file}`)
                .fetch() // `.read` for raw file, `.fetch` for JSON
                .then((res) => {
                    const isMarkdown = res.name.slice(-3) == '.md'
                    const contentString = window.atob(res.content)
                    const content = isMarkdown ?
                        convertToRaw(stateFromMarkdown(contentString)) :
...
function getToken(values) {
    const { token = auth.auth.getToken() } = values;
    if (token) {
        return new Promise((resolve) => resolve({ token }));
    } else {
        return auth.auth.gitHubPopupLogin();
    }
}
